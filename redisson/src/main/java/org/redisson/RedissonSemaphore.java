...
public class RedissonSemaphore extends RedissonExpirable implements RSemaphore {
    ...
    public void acquire(int permits) throws InterruptedException {
        if (tryAcquire(permits)) {   // 当前信号量资源数量足够，申请成功
            return;
        }
        // 当前信号量资源数量不够，线程阻塞，等待释放资源
        RFuture<RedissonLockEntry> future = subscribe();
        commandExecutor.syncSubscription(future);
        // 收到通知，循环尝试再次加锁
        try {
            while (true) {
                if (tryAcquire(permits)) {  // 获取资源成功,退出死循环
                    return;
                }
            ...
            unsubscribe(future);  // 获取资源成功，取消订阅
        }
//        get(acquireAsync(permits));
    ...
    public RFuture<Boolean> tryAcquireAsync(int permits) {
        ...
        // KEYS[1]：表示信号量的名称，即 semaphore
        // ARGV[1]：表示请求的信号量资源数量
        return commandExecutor.evalWriteAsync(getName(), LongCodec.INSTANCE, RedisCommands.EVAL_BOOLEAN,
                  "local value = redis.call('get', KEYS[1]); " +
                  "if (value ~= false and tonumber(value) >= tonumber(ARGV[1])) then " +  // 如果存在信号量，并且当前信号量资源数量大于等于申请的资源数量
                      "local val = redis.call('decrby', KEYS[1], ARGV[1]); " +  // 减掉申请的资源数量
                      "return 1; " +
                  "end; " +
                  "return 0;",
...
//        return get(tryAcquireAsync(permits, waitTime, unit));
    ...
    public RFuture<Void> releaseAsync(int permits) {
        ...
        // KEYS[1]：表示信号量的名称，即 semaphore
        // KEYS[2]：表示通道的名称，即 redisson_sc:{semaphore}
        // ARGV[1]：表示释放的信号量数量
        return commandExecutor.evalWriteAsync(getName(), StringCodec.INSTANCE, RedisCommands.EVAL_VOID,
            "local value = redis.call('incrby', KEYS[1], ARGV[1]); " +   // 增加信号量的资源数量
            "redis.call('publish', KEYS[2], value); ",      // 发布消息
    ...
    public RFuture<Boolean> trySetPermitsAsync(int permits) {
        ...
        // KEYS[1]：表示信号量的名称，即 semaphore
        // KEYS[2]：表示通道的名称，即 redisson_sc:{semaphore}
        // ARGV[1]：表示信号量同时能够允许获取锁的客户端的数量，即3
        return commandExecutor.evalWriteAsync(getName(), LongCodec.INSTANCE, RedisCommands.EVAL_BOOLEAN,
                "local value = redis.call('get', KEYS[1]); " +  // 获取信号量的值
                "if (value == false or value == 0) then "   // 信号量不存在 或者为 值为0
                    + "redis.call('set', KEYS[1], ARGV[1]); "   // 设置信号量的值
                    + "redis.call('publish', KEYS[2], ARGV[1]); "   // 发布通知消息
                    + "return 1;"
                + "end;"
                + "return 0;",
                Arrays.<Object>asList(getName(), getChannelName()), permits);
